...
//connection.connect();
//connection.query('use babysleep');
		...
		else if (signal == 9)
		{
			...
			if (totalTime >= 60)
			{
				...
				if (minutesTmp >= 60)
				{
					...
					var data = {date:date1, hour:hours, min:minutes, sec:seconds, wakeUpCnt:wakeUp};
					connection.query('insert into infosleep values(?,?,?,?,?)', data, function (error, result) {
					//connection.query('insert into infosleep(date, hour, min, sec, wakeUpCnt) values(?,?,?,?,?)', data, fuction(error, result) {
						if(!error){
							console.log(result);
						}
					});
				...
				else
				{
					...
					var data = [date1, 0, minutesTmp, seconds, wakeUp];
					connection.query('insert into infosleep(date, hour, min, sec, wakeUpCnt) values(?, ?, ?, ?, ?)', data, function(error, result){
						if(!error){
							console.log(result);
						}
					});	
